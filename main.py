...
for t, i, n in zip(func, init, name):
    ...
    graDescending.animate(coords, f, title=f"Vanilla_{n}", filename=f"Vanilla_{n}.gif")
    # print(coords[-10:-1])

    print("Adam")
    coords, evaluated = graDescending.Adam(f_var, i, lr=0.01, iter=iter)
    graDescending. animate(coords, f, title=f"Adam_{n}", filename=f"Adam_{n}.gif")
    # print(coords[-10:-1])

    print("RMSprop")
    coords, evaluated = graDescending.RMSprop(f_var, i, lr=0.01, iter=iter)
    graDescending.animate(coords, f, title=f"RMSprop_{n}", filename=f"RMSprop_{n}.gif")
    # print(coords[-10:-1])

    print("Momentum")
    coords, evaluated = graDescending.Momentum(f_var, i, lr=0.001, iter=iter)
    graDescending.animate(coords, f, title=f"Momentum_{n}", filename=f"Momentum_{n}.gif")
    # print(coords[-10:-1])

    print("BFGS")
    coords, evaluated = graDescending.BFGS(f_var, i, iter=iter)
    graDescending.animate(coords, f, title=f"BFGS_{n}", filename=f"BFGS_{n}.gif")
    # print(coords[-10:-1])
